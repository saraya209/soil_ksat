...
PTF_Ghanbarian_SHC2 <- function(sand, clay, bd, oc, L, dia, percent = TRUE){
  # Calculates saturated hydraulic conductivity using 
  #   Ghanbarian et al. (2015) SHC2 model.
  # 
  # Args:
  ...
  #   percent: Whether the size fractions are in ratio (0-1) or 
  #             in percentages (0-100).
  #
  # Returns:
  #   Named list 
  #      2. String of patterns matched, and 
  #      1. Predicted saturated hydraulic conducivity [cm/day]
  #
  ...
  ## Calculate geometric mean particle diameter, d_g (mm), and 
  ## geometric standard deviation of mean particle diameter, s_g (mm), 
  ## (Shirazi and Boersma, 1984)
  a <- 0.01 * ( (clay * log(0.001)) + (silt * log(0.026)) +(sand * log(1.025)) )
  
  b <- sqrt( 0.01 * ( (clay * log(0.001)^2) + 
                        (silt * log(0.026)^2) + 
                        (sand * log(1.025)^2) )  - 
               a^2)
  ...
  ### ---- Ghanbarian et al. (2015) SHC2 model ----
  # Steps:
  #   1. Match all relevant patterns (pattern == 0 if none )
  #   2. Estimate using all local models that match pattern OR baseline model
  #   3. Geometric mean of estimates weighted by ARR
  ...
  ## initialize all pattern predictions as NA
  K10 = K11 = K12 = K13 = K14 = K15 = K16 = K17 = K18 = K19 = K110 = K111 = K112 = K113 = K114 = NA 
  if ( 0 %in% pattern ){ ## basline regression model, $f_0$
    K10 = -1179.9976 + 11.8905 * sand + 11.9014 * silt + 11.8540 * clay + 
      5.2531 * d_g + 0.0279 * s_g -3.8603 * bd + -0.0387 * dia}
  if ( 1 %in% pattern){ 
    K11 = -1355.0792 + 14.0726 * sand + 9.9092 * silt + 0.0000 * clay - 
      50.1960 * d_g + 18.3884 * s_g - 12.9864 * bd - 0.2364 * dia - 0.0135 * L}
  if ( 2 %in% pattern ){ 
    K12 = 70.7377 - 0.5178 * sand - 0.6428 * silt + 0.0000 * clay - 
      30.3909 * d_g - 1.7316 * s_g - 4.6557 * bd + 2.6720 * dia - 0.0245 * L}
  if ( 3 %in% pattern ){ 
    K13 = 52.0616 + 0.5796 * sand - 0.5553 * silt + 0.0000 * clay - 
      230.6342 * d_g - 2.6810 * s_g - 1.1748 * bd + 0.3488 * dia - 0.0641 * L}
  if ( 4 %in% pattern ){ 
    K14 = -1033.6761 + 10.4057 * sand + 9.6609 * silt + 7.6045 * clay + 
      4.5560 * d_g + 4.5402 * s_g - 5.1402 * bd - 0.2013 * dia - 0.0006 * L}
  if ( 5 %in% pattern ){ 
    K15 = 53.5240 + 0.0949 * sand - 0.5390 * silt + 0.0000 * clay - 
      73.4111 * d_g - 1.5771 * s_g - 7.7699 * bd - 0.3555 * dia + 0.4150 * L}
  if ( 6 %in% pattern ){ 
    K16 = 762.4359 - 2.2777 * sand - 7.0926 * silt + 0.0000 * clay - 
      770.7954 * d_g - 22.6883 * s_g - 2.0712 * bd - 3.8762 * dia - 1.7193 * L}
  if ( 7 %in% pattern ){ 
    K17 = -50.4691 - 1.5926 * sand + 0.5580 * silt + 0.0000 * clay + 
      587.4053 * d_g + 7.4134 * s_g - 20.5499 * bd + 0.4336 * dia - 0.1798 * L}
  if ( 8 %in% pattern ){ 
    K18 = -243.3795 + 1.0718 * sand + 2.1596 * silt + 0.0000 * clay + 
      156.7400 * d_g + 5.0354 * s_g - 1.4163 * bd + 6.1225 * dia + 1.1349 * L}
  if ( 9 %in% pattern ){ 
    K19 = 13.7040 + 0.0419 * sand - 0.1043 * silt + 0.0000 * clay - 
      60.0944 * d_g - 0.0354 * s_g - 2.8371 * bd + 0.1564 * dia - 0.0114 * L}
  if ( 10 %in% pattern ){ 
    K110 = 281.9100 - 1.3948 * sand - 2.3683 * silt + 0.0000 * clay - 
      142.4181 * d_g - 5.2810 * s_g - 22.6087 * bd + 0.4585 * dia - 2.8566 * L}
  if ( 11 %in% pattern ){ 
    K111 = 148.3718 - 0.4490 * sand - 1.2957 * silt + 0.0000 * clay - 
      145.3112 * d_g - 4.2241 * s_g - 4.2887 * bd - 0.0533 * dia - 0.0857 * L}
  if ( 12 %in% pattern ){ 
    K112 = -896.5401 + 9.2031 * sand + 9.0852 * silt + 9.7194 * clay - 
      5.3507 * d_g - 1.3652 * s_g - 4.6936 * bd - 0.1804 * dia - 0.0005 * L}
  if ( 13 %in% pattern ){ 
    K113 = 76.0376 + 0.1459 * sand - 0.6629 * silt + 0.0000 * clay - 
      144.6754 * d_g - 3.1245 * s_g - 1.3179 * bd + 0.0301 * dia - 0.0606 * L}
  if ( 14 %in% pattern ){ 
    K114 = -204.8986 + 2.1700 * sand + 2.1471 * silt + 2.3956 * clay + 
      2.5379 * d_g - 0.3229 * s_g - 3.7845 * bd - 0.1985 * dia + 0.0861 * L}
  ...
  vwts = c(1, 1.4726, 2.2554, 3.0696, 0.9416, 6.5948, 1.6394, 3.3942, 2.051, 2.3825, 4.276, 0.8731, 0.8721, 0.5489, 0.3524)
...
Brakensiek_ks <- function(sand, clay, bd, percent = TRUE){
  # Calculates saturated hydraulic conductivity 
  # using Brakensiek et al. (1984) model
  # 
  # Args:
  #   sand,clay,...: Ratio or percent of particles of size sand and clay
  #   bd: bulk density of soil (g/cm^3)
  ...
  #
  # Returns: Saturated hydraulic conducivity [(cm/day)]
  #
  # Convert ratio to percent
  if(percent == FALSE){
    sand = 100*sand
  ...
  phi <- 1 - (bd / 2.65)
  #
  ### Brakensiek et al. (1984) model (verified)
  K2 = exp(19.52348 * phi
           - 8.96847
           - 0.028212 * clay
           + 0.00018107 * (sand^2)
...
CampbellShiozawa_ks <- function(sand, clay, percent = TRUE){
  # Calculates saturated hydraulic conductivity 
  # using Campbell & Shiozawa (1994) model
  # 
  # Args:
  #   sand,clay,...: Ratio or percent of particles of size sand and clay
  #   percent: Whether the size fractions are in ratio (0-1)
  ...
  silt = 100-sand-clay  # Calculate silt percent
  #
  ### Campbell & Shiozawa (1994) model (verified)
...
Cosby_ks <- function(sand, clay, percent = TRUE){
  # Calculates saturated hydraulic conductivity 
  # using Cosby et al. (1984) model
  # 
  # Args:
  #   sand,clay,...: Ratio or percent of particles of size sand and clay
  #   percent: Whether the size fractions are in ratio (0-1)
  ...
  silt = 100-sand-clay  # Calculate silt percent
  #
  ### Cosby et al. (1984) model (verified)
...
Jabro_ks <- function(sand, clay, bd, percent = TRUE){
  # Calculates saturated hydraulic conductivity 
  # using Jabro (1992) model
  # 
  # Args:
  #   sand,clay,...: Ratio or percent of particles of size sand and clay
  #   bd: bulk density of soil (g/cm^3)
  ...
  silt = 100-sand-clay  # Calculate silt percent
  #
  ### Jabro (1992) model (verified)
  if ( silt < 0.1 | clay < 0.1){
    K5 = -99
  }else{
    K5 = 10^(9.56 - 0.81 * log10(silt) - 1.09 * log10(clay) - 4.64 * bd)
    # convert cm/hr to cm/day
    K5 = K5 * 24
  }
...
Pucket_ks <- function(clay, percent = TRUE){
  # Calculates saturated hydraulic conductivity 
  # using Pucket et al. (1985) model
  # 
  # Args:
  #   clay,...: Ratio or percent of particles of size clay
  #   percent: Whether the size fractions are in ratio (0-1)
  #             or in percentages (0-100).
  #
  # Returns: Saturated hydraulic conducivity [(cm/day)]
  ...
  #
  ### Pucket et al. (1985) model (verified)
...
DanePucket_ks <- function(clay, percent = TRUE){
  # Calculates saturated hydraulic conductivity 
  # using Dane & Puckett (1994) mode
  # 
  # Args:
  #   clay,...: Ratio or percent of particles of size clay
  #   percent: Whether the size fractions are in ratio (0-1)
  ...
  #
  ### Dane & Puckett (1994) model (verified)
...
Nemes_ks <- function(sand, clay, bd, oc, oc_om_factor = 1.724, is_oc = TRUE, percent = TRUE){
  # Calculates saturated hydraulic conductivity 
  # using Nemes et al. (2005)
  # 
  # Args:
  #   sand,clay,...: Ratio or percent of particles of size sand and clay
  #   bd: bulk density of soil (g/cm^3)
  #   oc: organic carbon (or matter) content (%)
  #   oc_om_factor: Organic carbon to organic matter conversion factor
  #   is_oc: is value supplied to oc carbon? if False, its organic matter.
  #   percent: Whether the size fractions are in ratio (0-1)
  #             or in percentages (0-100).
  #
  # Returns: Saturated hydraulic conducivity [(cm/day)]
  ...
  # OC to OM conversion:
  if (is_oc){
    om = oc * oc_om_factor ## 1.724 most commonly used conversion.
